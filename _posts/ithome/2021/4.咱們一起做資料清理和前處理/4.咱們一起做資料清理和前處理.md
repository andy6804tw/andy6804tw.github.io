...
## 哪些機器學習算法不需要做標準化處理?
### 需要標準化的模型
在實際應用中，通過梯度下降法求解的模型一般都是需要歸一化的，比如線性回歸、logistic回歸、KNN、SVM、神經網絡等模型。
1. 基於距離計算的模型： KNN、K-means、PCA 
2. 通過梯度下降求解的模型：線性迴歸、邏輯迴歸、支持向量機、神經網路

### 不需要標準化的模型
但樹模型不需要標準化，因為它們不關心變量的值，而是關心變量的分佈和變量之間的條件概率，如決策樹、隨機森林...等。

1. 與距離計算無關的機率模型：Naive Bayes
2. 樹模型：決策樹、隨機森林、Gradient boosting、XGBoost

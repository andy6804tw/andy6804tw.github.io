...
```py
#直方圖 histograms
df_train.hist(alpha=0.6,layout=(3,3), figsize=(12, 8), bins=10) 
plt.tight_layout()
plt.show()
```
...
sns.histplot(df_train["SepalLengthCm"][:],ax=axes[0], kde=True)
sns.histplot(df_train["SepalWidthCm"][:],ax=axes[1], kde=True)
sns.histplot(df_train["PetalLengthCm"][:],ax=axes[2], kde=True)
sns.histplot(df_train["PetalWidthCm"][:],ax=axes[3], kde=True)
```
...
使用 Seaborn 繪製：
```py
sns.pairplot(df_train, hue="Species", height=2, diag_kind="kde")
```
...
```py
# correlation 計算
corr = df_train[['SepalLengthCm','SepalWidthCm','PetalLengthCm','PetalWidthCm','Species']].corr()
...
```py
sns.lmplot("SepalLengthCm", "SepalWidthCm", hue='Species', data=df_train, fit_reg=False, legend=False)
